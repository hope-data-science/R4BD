...
我们在前面两个章节中使用了R语言生态中的一些大数据方案，需要明确的是，如果使用`qs::qsave`和`saveRDS`函数所写出的文件，只有R环境才能再次进行读取，使用其他软件则一般没有通用接口可以读入。而我们在“速度至上”章节中发现的最快格式fst，当前也只是在R中比较流行，这种格式在其他软件平台（如Python）还没有接口能够读入。
